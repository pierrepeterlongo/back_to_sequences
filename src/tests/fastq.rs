...
pub fn record(rng: &mut rand::rngs::StdRng, comment_length: usize, read_length: usize) -> Vec<u8> {
    let mut result = Vec::with_capacity(comment_length + 7 + (read_length + 2) * 2 + 2);

    result.extend(comment(rng, comment_length));
    result.extend(super::sequence(rng, read_length));
    result.extend(b"\n+\n");
    result.extend(super::quality(rng, read_length));
    result.push(b'\n');
...
pub fn records(
    ...
    let mut result = Vec::with_capacity((comment_length + 7 + (read_length + 2) * 2 + 2) * n);
