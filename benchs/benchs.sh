...
do
    echo "nb_reads: $nb_reads"
    time back_to_sequences --in-kmers compacted_kmers.fasta --in-sequences reads_${nb_reads}.fasta  --out-sequences filtered_reads_${nb_reads}.fasta  -k 31 --out-kmers counted_kmers_${nb_reads}.txt --output-kmer-positions 
    ...
    # # check that kmer_positions_${nb_reads}.txt kmer_positions_only_${nb_reads}.txt are the same
    # diff kmer_positions_${nb_reads}.txt kmer_positions_only_${nb_reads}.txt > /dev/null
    # if [ $? -eq 0 ]
    # then
    #     echo "OK"
    # else
